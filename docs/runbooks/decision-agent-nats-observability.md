# Decision Agent NATS Observability Runbook

Use this runbook to inspect Decision Agent event publishing and NATS telemetry.

## What the agent publishes
- Subject: `agent.decision.audit`
- Source tag: `agents.decision_agent`
- Envelope fields: `id`, `ts`, `actor`, `family_id`, `type`, `payload`, `source`

Current audit event names emitted by the Decision Agent:
- `plan_created`
- `plan_executed`
- `execution_failure`
...
2. Live tail only Decision Agent audit telemetry:
...
1. Trigger a normal request and confirm `payload.event=plan_created` appears.
